...
{         //$clean_email  = val_email($_POST['email']);
...
{   //$hashed_password  = hash_pwd($_POST['password']);
...
function process_registration()
{
    ...
    if (isset($_POST['submit_registration'])) {
        $cl_name        =   santize(($_POST['name']));
        $cl_sex         =   santize(($_POST['sex']));
        $cl_email       =   val_email(($_POST['email']));
        $cl_password    =   santize(($_POST['password']));
        ...
        if (!in_array($image_ext, $allowed_image)) {
            ...
            if ($get_user > 0) {
                ...
                if ($run) {
                    ...
                    send_email($cl_email, $cl_password);


                    set_msg('<div class="alert alert-success text-center">
                      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                      <strong>Success ' . $cl_name . '!</strong> Registration was successfull. Please Login. Registration informations were send on email account <strong>' . $cl_email .' </strong> 
                    </div>');
                } else {
